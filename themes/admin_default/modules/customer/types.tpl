    ...
    <div class="panel panel-default col-md-24">
        <div class="panel-body">
            ...
            <ul class="nav nav-tabs" role="tablist">
				...
				<li role="presentation"><a href="#accumulated_points" aria-controls="accumulated_points" role="tab" data-toggle="tab">{LANG.accumulated_points}</a></li>
            </ul>
            <div class="tab-content">
				...
				<div role="tabpanel" class="tab-pane" id="accumulated_points">
                    <div role="tabpanel" class="tab-pane active" id="accumulated_points">
                        <div class="col-sm-24 col-md-24">
                            <label class="control-label"><strong>{LANG.title}</strong> <span class="red">(*)</span></label> <input class="form-control" type="text" name="accumulated_points_title" id="accumulated_points_title" value="{ROW.accumulated_points_title}" />
                        </div>
                        <div class="col-sm-24 col-md-24">
                            <label class="control-label"><strong>{LANG.content}</strong></label>
                            <div>{ROW.accumulated_points_content}</div>
                        </div>
                    </div>
                </div>
...
//<![CDATA[
	function nv_change_weight(id) {
		...
		$.post(script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=types&nocache=' + new Date().getTime(), 'ajax_action=1&id=' + id + '&new_vid=' + new_vid, function(res) {
			var r_split = res.split('_');
			if (r_split[0] != 'OK') {
				alert(nv_is_change_act_confirm[2]);
			}
			window.location.href = script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=types';
			return;
		});
		return;
	}
	...
	function nv_change_status(id) {
		...
		if (confirm(nv_is_change_act_confirm[0])) {
			...
			$.post(script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=types&nocache=' + new Date().getTime(), 'change_status=1&id='+id, function(res) {
				var r_split = res.split('_');
				if (r_split[0] != 'OK') {
					alert(nv_is_change_act_confirm[2]);
				}
