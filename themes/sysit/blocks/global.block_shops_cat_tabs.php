 ...
 * @Author NV SYSTEMS (adminwmt@gmail.com)
...
{
	function nvs_viewsub($list_sub, $block_config, $nvs_array_cat_shops, $data_block)
    {
            ...
            foreach ($list as $catid) {
                if ($nvs_array_cat_shops[$catid]['inhome'] == '1') {
                    ...
                    if (!empty($nvs_array_cat_shops[$catid]['subcatid'])) {
                        $html .= nvs_viewsub($nvs_array_cat_shops[$catid]['subcatid'], $block_config, $nvs_array_cat_shops, $data_block);
                    }
                    $html .= "";
                }
            }
	...
	function nv_block_config_shops_block_cat_tabs( $module, $data_block, $lang_block )
	{
        ...
        if ($nv_Request->isset_request('loadajaxdata', 'get')) {
				...
				if ($cat['parentid'] == 0) {
					if ($cat['inhome'] == '1') {
						...
						if (!empty($cat['subcatid'])) {
							...
							$html .= nvs_viewsub($cat['subcatid'], $block_config, $nvs_array_cat_shops, $data_block['blockid']);
						}
						$html .= "";
					}
				}
			...
			$html .= '<tr>';
			$html .= '<td>' . $lang_block['display_type'] . '</td>';
			$html .= '<td><input type="checkbox" value="1" name="config_display_type" ' . ($data_block['display_type'] == 1 ? 'checked="checked"' : '') . ' /></td>';
			$html .= '</tr>';
			
			$html .= '<tr>';
			$html .= '<td>' . $lang_block['blockwidth'] . '</td>';
			$html .= '<td><input type="text" value="' . $data_block['blockwidth'] . '" name="config_blockwidth"  /></td>';
			$html .= '</tr>';
			
			$html .= '<tr>';
			$html .= '<td>' . $lang_block['blockheight'] . '</td>';
			$html .= '<td><input type="text" value="' . $data_block['blockheight'] . '" name="config_blockheight"  /></td>';
			$html .= '</tr>';
			
			$html .= '<tr>';
			$html .= '<td>' . $lang_block['show_no_image'] . '</td>';
			$html .= '<td><input type="text" value="' . $data_block['show_no_image'] . '" name="config_show_no_image"  /></td>';
			$html .= '</tr>';
			nv_htmlOutput($html);
		}
		$html = '<tr>';
	...
	if (!nv_function_exists('nv_systems_get_price')) {
        function nv_systems_get_price($pro_id, $currency_convert, $number = 1, $per_pro = false, $module = '')
		{
			...
			// Lay ty gia ngoai te
			$sql = 'SELECT code, currency,symbol, exchange, round, number_format FROM ' . $db_config['prefix'] . '_shops_money_' . NV_LANG_DATA;
			...
			// Lay Giam Gia
			$sql = 'SELECT did, title, begin_time, end_time, config FROM ' . $db_config['prefix'] . '_shops_discounts';
			...
			$product = $db->query('SELECT listcatid, product_price, money_unit, price_config, discount_id FROM ' . $db_config['prefix'] . '_' . $module . '_rows WHERE id = ' . $pro_id)->fetch();
	...
	if (!nv_function_exists('nv_systems_get_price_tmp')) {
        function nv_systems_get_price_tmp($module_name, $module_data, $module_file, $pro_id)
        {
            global $nv_Cache, $db, $db_config, $module_config;
            $price = array();
	...
	if (!nv_function_exists('NVSGetGroupID')) {
		function NVSGetGroupID($block_config, $pro_id, $group_by_parent = 0 )
		{
			global $db, $db_config, $module_data;
			$module = $mod_data = $mod_file = $module_name = $module_data = $module_file = $block_config['selectmod'];
	...
	function nv_block_shops_block_cat_tabs( $block_config )
	{
		...
		$sql = 'SELECT ' . NV_LANG_DATA . '_alias AS alias FROM ' . $db_config['prefix'] . '_' . $module . '_catalogs WHERE catid IN (' . $blockid . ') and parentid = 0 ORDER BY sort ASC';
		$result=$db->query($sql);
		$cat_block=$result->fetch();
		$cat_link_block = NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=shops&amp;' . NV_OP_VARIABLE . '=' . $cat_block['alias'] . $global_config['rewrite_exturl'];
		
		if( file_exists( NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/blocks/global.block_shops_cat_tabs.tpl' ) )
		{
		...
		$xtpl = new XTemplate( 'global.block_shops_cat_tabs.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/blocks' );
		...
		$xtpl->assign( 'NVS_TITLE', $block_config['title']);
			...
			$db->sqlreset()
				->select( 't1.id, t1.listcatid, t1.'. NV_LANG_DATA .'_title as title, t1.'. NV_LANG_DATA .'_alias as alias, t1.homeimgfile, t1.homeimgthumb,t1.'. NV_LANG_DATA .'_hometext as hometext,t1.publtime, t1.showprice, t1.discount_id, t1.product_number, t1.money_unit, t1.product_code, t3.newday, t3.'. NV_LANG_DATA .'_alias as catalias' )
				->from( $db_config['prefix'] . '_shops_rows t1' )
				->join( 'INNER JOIN ' . $db_config['prefix'] . '_shops_catalogs t3 ON t3.catid = t1.listcatid' )
				->where( 't3.catid IN (' . $blockid . ') AND t1.status= 1' )
				->order( 't1.id ASC' )
			...
			if( ! empty( $list ) )
			{
				while( $l = $list->fetch() )
				{
					$l['link'] = NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=shops&amp;' . NV_OP_VARIABLE . '=' . $l['catalias'] . '/' . $l['alias'] . $global_config['rewrite_exturl'];
					...
					if ($pro_config['active_order'] == '1' and $pro_config['active_order_non_detail'] == '1') {
						if ($l['showprice'] == '1') {
							if ($l['product_number'] > 0) {
                                ...
                                $listgroupid = NVSGetGroupID($block_config, $l['id']);
                                $group_requie = 0;
					...
					if ($pro_config['active_price'] == '1') {
						if ($l['showprice'] == '1') {
							$price = nv_systems_get_price_tmp($module, $mod_data, $mod_file, $l['id']);
		...
		if($block_config['display_type'] ==1){
			$xtpl->assign('cat_link_block', $cat_link_block);
			$xtpl->parse( 'grid' );
