...
if (!nv_function_exists('nv_room_info')) {
    /**
     ...
     * @return string
     */
    function nv_room_info_config($module, $data_block, $lang_block)
    {
		...
		$lang_global['company_hour'] = $lang_block['company_hour'];
		$lang_global['company_class'] = $lang_block['company_class'];
        ...
        $html .= '</div>';
		$html .= '<div class="form-group">';
        $html .= '<label class="control-label col-sm-6">' . $lang_global['company_hour'] . ':</label>';
        $html .= '<div class="col-sm-18"><input type="text" class="form-control" name="config_company_hour" value="' . $data_block['company_hour'] . '"></div>';
        $html .= '</div>';
		$html .= '<div class="form-group">';
        $html .= '<label class="control-label col-sm-6">' . $lang_global['company_class'] . ':</label>';
        $html .= '<div class="col-sm-18"><input type="text" class="form-control" name="config_company_class" value="' . $data_block['company_class'] . '"></div>';
        $html .= '</div>';
        $html .= '<div class="form-group">';
    ...
    function nv_room_info_submit()
    {
        ...
        $return['config']['company_hour'] = $nv_Request->get_title('config_company_hour', 'post');
        $return['config']['company_class'] = $nv_Request->get_title('config_company_class', 'post');
     ...
     * @return string
     */
    function nv_room_info($block_config)
    {
        global $global_config, $lang_global;

        if (file_exists(NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/blocks/global.room_info.tpl')) {
            $block_theme = $global_config['module_theme'];
        } elseif (file_exists(NV_ROOTDIR . '/themes/' . $global_config['site_theme'] . '/blocks/global.room_info.tpl')) {
            $block_theme = $global_config['site_theme'];
        } else {
            $block_theme = 'default';
        }

        $xtpl = new XTemplate('global.room_info.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/blocks');
        $xtpl->assign('LANG', $lang_global);
        $xtpl->assign('NV_BASE_SITEURL', NV_BASE_SITEURL);
        $xtpl->assign('DATA', $block_config);
        $xtpl->assign('CLASS', $block_config['company_class']);
		...
		if (!empty($block_config['company_hour'])) {
			$xtpl->assign('HOUR', $block_config['company_hour']);
            $xtpl->parse('main.company_hour');
        }
...
if (defined('NV_SYSTEM')) {
    $content = nv_room_info($block_config);
}
