...
<form class="form-horizontal" action="{NV_BASE_SITEURL}index.php?{NV_LANG_VARIABLE}={NV_LANG_DATA}&amp;{NV_NAME_VARIABLE}={MODULE_NAME}&amp;{NV_OP_VARIABLE}={OP}" method="post" id="frm-submit" enctype="multipart/form-data">
    <div class="panel panel-default">
        <div class="panel-body">
            ...
            <div class="form-group" id="list_category">
                <label class="col-sm-5 col-md-4 control-label"><strong>{LANG.sl_category}</strong></label>
                <div class="col-sm-19 col-md-20">
                    <select class="form-control select2" name="catid" style="width: 100%">
                        <option value="0">---{LANG.sl_category}---</option>
                        <!-- BEGIN: list_category -->
                        <option value="{CATEGORY.id}"{CATEGORY.sl}>{CATEGORY.title}</option>
                        <!-- END: list_category -->
                    </select>
                </div>
            </div>
            <div class="form-group">
                <label class="col-sm-5 col-md-4 control-label"><strong>{LANG.progress}</strong></label>
                <div class="col-sm-19 col-md-20">
                    <input class="form-control" type="number" name="progress" id="progress" value="{ROW.progress}" oninvalid="setCustomValidity( nv_required )" oninput="setCustomValidity('')" />
                </div>
            </div>
            <!-- BEGIN: allow_useradd -->
...
<script type="text/javascript">
    ...
    $('#frm-submit').submit(function(e) {
        ...
        $.ajax({
            type : 'POST',
            url : '/index.php?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=content&nocache=' + new Date().getTime(),
            data : data,
            dataType: 'json',
            processData : false,
            contentType : false,
