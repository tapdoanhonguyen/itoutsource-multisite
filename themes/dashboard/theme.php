...
function nv_site_theme( $contents, $full = true )
{
	...
	$xtpl->assign( 'MODULE_TITLE', $module_info['site_title'] );
 
	
	global $user_info;
	
	$user_info['photo']= isset( $user_info['photo'] ) ? NV_BASE_SITEURL . $user_info['photo'] : '';
	...
	{
		$metatags[] = ['name' => 'name', 'value' => 'viewport', 'content' => 'width=device-width, initial-scale=1'];
	}
	...
	$html_links[] = ['rel' => 'stylesheet', 'href' => NV_BASE_SITEURL . 'themes/' . $global_config['module_theme'] . '/css/custom.css'];
	...
	if( isset( $module_config['themes'][$global_config['module_theme']] ) and ! empty( $module_config['themes'][$global_config['module_theme']] ) )
	{
		...
		if( isset( $config_theme['css_content'] ) and ! empty( $config_theme['css_content'] ) )
		{
			...
			{
				$replace = ['[body]' => 'body', '[a_link]' => 'a, a:link, a:active, a:visited', '[a_link_hover]' => 'a:hover', '[content]' => '.wraper', '[header]' => '#header', '[footer]' => '#footer', '[block]' => '.panel, .well, .nv-block-banners', '[block_heading]' => '.panel-default > .panel-heading'];
			...
			$html_links[] = ['rel' => 'stylesheet', 'href' => NV_BASE_SITEURL . NV_ASSETS_DIR . '/css/' . $customFileName . '.css'];
		}
	...
	$html_js[] = ['ext' => 1, 'content' => NV_BASE_SITEURL . 'themes/' . $global_config['module_theme'] . '/js/custom.js'];
	...
	if( $module_name == 'users' && ( $op == 'login' || $op == 'logout' ) )
	{
		$xtpl->assign( 'CLASS_BODY', 'hold-transition login-page' );
	}
	else
	{
		$xtpl->assign( 'CLASS_BODY', 'hold-transition sidebar-mini layout-fixed' );
	}
	...
	if( $full )
	{
		...
		if( ! $home )
		{
			...
			{
				$arr_cat_title_i = ['catid' => 0, 'title' => $module_info['custom_title'], 'link' => NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name];
		...
		if( sizeof( $global_config['array_theme_type'] ) > 1 )
		{
			...
			$icons = ['r' => 'random', 'd' => 'desktop', 'm' => 'mobile'];
