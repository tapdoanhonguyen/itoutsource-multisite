...
$(function () {
    if ($('#frmEstimate') && $('.btnEstimate').length > 0) {
        $('.btnEstimate').on('click', function () {
            $('#frmEstimate').modal('hide');
            $.ajax({
                type: "POST",
                cache: !1,
                url: nv_base_siteurl + "index.php?" + nv_lang_variable + "=" + nv_lang_data + "&" + nv_name_variable + "=" + $('#frmEstimate').data("module"),
                data: "loadForm=1&checkss=" + $('#frmEstimate').data("cs") + "&pid=" + $(this).data('product') + "&type=" + $(this).data('type'),
                dataType: "html",
                success: function (a) {
                    $('#frmEstimate .modal-content').html(a);
                    change_captcha();
                    $('#frmEstimate').modal({
                        backdrop: 'static'
                    }).show();
