...
if (! nv_function_exists('nv_menu_bootstrap_mobi')) {
    /**
     * nv_menu_bootstrap_mobi_check_current()
     *
     * @param mixed $url
     * @param integer $type
     * @return
     *
     */
    function nv_menu_bootstrap_mobi_check_current($url, $type = 0)
    {
    ...
    /**
     * nv_menu_bootstrap_mobi()
     *
     * @param mixed $block_config
     * @return
     *
     */
    function nv_menu_bootstrap_mobi($block_config)
    {
        global $nv_Cache, $global_config, $site_mods, $module_info, $module_name, $module_file, $module_data, $lang_global, $catid, $home;

        if (file_exists(NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/blocks/global.bootstrap_mobi.tpl')) {
            $block_theme = $global_config['module_theme'];
        } elseif (file_exists(NV_ROOTDIR . '/themes/' . $global_config['site_theme'] . '/blocks/global.bootstrap_mobi.tpl')) {
        ...
        $xtpl = new XTemplate('global.bootstrap_mobi.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/blocks');
        ...
        if (!empty($array_menu)) {
            foreach ($array_menu[0] as $id => $item) {
                ...
                if (isset($array_menu[$id])) {
                    ...
                    $submenu = nv_get_bootstrap_mobi_submenu($id, $array_menu, $submenu_active, $block_theme);
                    $xtpl->assign('SUB', $submenu);
                    $xtpl->parse('main.top_menu.sub');
                    $xtpl->parse('main.top_menu.has_sub');
                }
                if (nv_menu_bootstrap_mobi_check_current($item['link'], $item['active_type'])) {
                    $classcurrent[] = 'active';
     ...
     * @return string
     */
    function nv_get_bootstrap_mobi_submenu($id, $array_menu, &$submenu_active, $block_theme)
    {
        $xtpl = new XTemplate('global.bootstrap_mobi.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/blocks');

        if (! empty($array_menu[$id])) {
            foreach ($array_menu[$id] as $sid => $smenu) {
                if (nv_menu_bootstrap_mobi_check_current($smenu['link'], $smenu['active_type'])) {
                    $submenu_active[] = $id;
                }
                $submenu = '';
                if (isset($array_menu[$sid])) {
                    $submenu = nv_get_bootstrap_mobi_submenu($sid, $array_menu, $submenu_active, $block_theme);
                    $xtpl->assign('SUB', $submenu);
...
if (defined('NV_SYSTEM')) {
    $content = nv_menu_bootstrap_mobi($block_config);
}