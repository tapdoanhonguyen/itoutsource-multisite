...
$page_title = $lang_module['work_place'];

function work_place_fix_weight()
{
	global $db;
	$sql = 'SELECT data_id FROM ' . TABLE_TIMEKEEPING_NAME . '_data WHERE group_name=\'work_place\' ORDER BY weight ASC';
	$result = $db->query( $sql );
	$weight = 0;
	while( $row = $result->fetch() )
	{
		++$weight;
		$db->query( 'UPDATE ' . TABLE_TIMEKEEPING_NAME . '_data SET weight=' . $weight . ' WHERE group_name=\'work_place\' AND data_id=' . $row['data_id'] );
	}
	$result->closeCursor();
}
...
if( ACTION_METHOD == 'delete' )
{
	...
	if( ! empty( $del_array ) )
	{
		...
		foreach( $del_array as $data_id )
		{
			...
			$db->query( 'DELETE FROM ' . TABLE_TIMEKEEPING_NAME . '_data WHERE group_name=\'work_place\' AND data_id = ' . ( int )$data_id );
		...
		if( $count )
		{
			...
			$json['success'] = $lang_module['work_place_delete_success'];
		}

	}
	else
	{
		$json['error'] = $lang_module['work_place_error_security'];
	}
...
elseif( ACTION_METHOD == 'status' )
{
	...
	{
		$sql = 'UPDATE ' . TABLE_TIMEKEEPING_NAME . '_data SET status=' . $new_vid . ' WHERE group_name=\'work_place\' AND data_id=' . $data_id;
		if( $db->exec( $sql ) )
		{
			...
			$json['success'] = $lang_module['work_place_status_success'];

		}
		else
		{
			$json['error'] = $lang_module['work_place_error_status'];
...
elseif( ACTION_METHOD == 'weight' )
{
	...
	if( $token == md5( $nv_Request->session_id . $global_config['sitekey'] . $data_id ) )
	{
		$sql = 'SELECT data_id FROM ' . TABLE_TIMEKEEPING_NAME . '_data WHERE group_name=\'work_place\' AND data_id !=' . $data_id . ' ORDER BY weight ASC';
		$result = $db->query( $sql );
		...
		while( $row = $result->fetch() )
		{
			...
			$sql = 'UPDATE ' . TABLE_TIMEKEEPING_NAME . '_data SET weight=' . $weight . ' WHERE group_name=\'work_place\' AND data_id=' . intval( $row['data_id'] );
			$db->query( $sql );
		}

		$sql = 'UPDATE ' . TABLE_TIMEKEEPING_NAME . '_data SET weight=' . $new_vid . ' WHERE group_name=\'work_place\' AND data_id=' . $data_id;
		if( $db->exec( $sql ) )
		{
			...
			$json['success'] = $lang_module['work_place_weight_success'];
			$json['link'] = NV_BASE_ADMINURL . 'index.php?' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=' . $op;

		}
		else
		{
			$json['error'] = $lang_module['work_place_error_weight'];
...
elseif( ACTION_METHOD == 'add' || ACTION_METHOD == 'edit' )
{
	...
	if( $data['data_id'] > 0 )
	{
		...
		FROM ' . TABLE_TIMEKEEPING_NAME . '_data  
		WHERE data_id=' . $data['data_id'] )->fetch();

		$caption = $lang_module['work_place_edit'];
	}
	else
	{
		$caption = $lang_module['work_place_add'];
	}
	...
	if( $nv_Request->get_int( 'save', 'post' ) == 1 )
	{
		...
		if( empty( $data['title'] ) )
		{
			$error['title'] = $lang_module['work_place_error_title'];
		}
 

		if( ! empty( $error ) && ! isset( $error['warning'] ) )
		{
			$error['warning'] = $lang_module['work_place_error_warning'];
		}
		...
		if( empty( $error ) )
		{
			...
			if( $data['data_id'] == 0 )
			{
				try
				{
					$stmt = $db->prepare( 'SELECT MAX(weight) FROM ' . TABLE_TIMEKEEPING_NAME . '_data WHERE group_name=' . $db->quote( $data['group_name'] ) );
					$stmt->execute();
					$weight = $stmt->fetchColumn();

					$weight = intval( $weight ) + 1;

					$stmt = $db->prepare( 'INSERT INTO ' . TABLE_TIMEKEEPING_NAME . '_data SET 
					...
					{
						nv_insert_logs( NV_LANG_DATA, $module_name, $lang_module['work_place_add'], 'data_id: ' . $data['data_id'], $admin_info['userid'] );
						
						$nv_Request->set_Session( $module_data . '_success', $lang_module['work_place_insert_success'] );
					}
					else
					{
						$error['warning'] = $lang_module['work_place_error_save'];
			...
			else
			{
				try
				{
					...
					$stmt = $db->prepare( 'UPDATE ' . TABLE_TIMEKEEPING_NAME . '_data SET 
						status=' . intval( $data['status'] ) . ',
						title =:title,
						group_name =:group_name
					...
					if( $stmt->execute() )
					{
						nv_insert_logs( NV_LANG_DATA, $module_name, $lang_module['work_place_edit'], 'data_id: ' . $data['data_id'], $admin_info['userid'] );
						$nv_Request->set_Session( $module_data . '_success', $lang_module['work_place_update_success'] );
					}
					else
					{
						$error['warning'] = $lang_module['work_place_error_save'];
	...
	$xtpl = new XTemplate( 'work_place_add.tpl', NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/' . $module_file );
...
/*show list work_place*/

$per_page = 50;

$page = $nv_Request->get_int( 'page', 'get', 1 );

$sql = TABLE_TIMEKEEPING_NAME . '_data';

$implode[] = 'group_name=\'work_place\'';
...
$base_url = NV_BASE_ADMINURL . 'index.php?' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=work_place&amp;sort=' . $sort . '&amp;order=' . $order . '&amp;per_page=' . $per_page;
...
$xtpl = new XTemplate( 'work_place.tpl', NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/' . $module_file );
$xtpl->assign( 'LANG', $lang_module );
$xtpl->assign( 'NV_LANG_VARIABLE', NV_LANG_VARIABLE );
$xtpl->assign( 'NV_LANG_DATA', NV_LANG_DATA );
$xtpl->assign( 'NV_BASE_SITEURL', NV_BASE_SITEURL );
...
$xtpl->assign( 'ADD_NEW', NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . "&" . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=work_place&action=add' );
