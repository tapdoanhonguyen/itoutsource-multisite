...
if (defined('NV_IS_USER') && in_array($module_config[$module_name]['employer_group'],$user_info['in_groups']) || defined('NV_IS_ADMIN')){
	$array_data = array();
	$row = array();
	$error = array();
	...
	if ($nv_Request->isset_request('submit', 'post')) {
		...
		$row = $db->query('SELECT id FROM ' . NV_PREFIXLANG . '_' . $module_data . ' WHERE userid=' . $row['userid'] . ' AND date_login >= ' . $date_current_start . ' AND date_login <= ' . $date_current_end)->fetchAll();
		if (empty($row)) {
			nv_redirect_location(NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=' . $op);
		}
	...
	$contents = nv_theme_timekeeper_shift($array_data, $array_locationid,  $module_config, $module_name);
