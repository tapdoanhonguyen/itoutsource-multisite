...
if( ACTION_METHOD == 'Export' )
{
	...
	if( $token == $nv_Request->session_id . $global_config['sitekey'] )
	{
		...
		$work_status = $nv_Request->get_int('work_status', 'post', 0);
		...
		$implode = array();
		if( !empty( $work_status ) && $all == 0)
		{
			$implode[] = 'work_status=' . intval( $work_status );
...
{
	$json['template'] = ThemeViewWorkforceExport( );
...
if( ACTION_METHOD == 'getData' )
{
	...
	$json['data'] = array();
	$work_status = $nv_Request->get_int('work_status', 'get', 0);
	$work_status = ( $work_status <= 3 ) ? $work_status : 0;
...
$dataCount = array(); 
$result = $db->query('SELECT COUNT(*) total, work_status FROM ' . TABLE_PERSONNEL_NAME .  '_personnel GROUP BY work_status');
$total = 0;
while( $item = $result->fetch() )
{
	$total = $total + intval($item['total']);
	$dataCount[$item['work_status']] = intval( $item['total'] );
}
$dataCount[0] = $total;
$result->closeCursor();

$contents = ThemeViewWorkforce( $dataCount );
