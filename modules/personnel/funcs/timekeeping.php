...
if( ACTION_METHOD == 'usersreport' )
{
 
	
	$row = array();
	$users_reports = $nv_Request->get_int('userslist', 'post,get', 0);
	//$users_reports[] = 4;
	//$users_reports[] = 5;
	$date_begin_timekeeping = $nv_Request->get_string('date_begin_report', 'post,get', date("d/m/Y",NV_CURRENTTIME));
	$date_end_timekeeping = $nv_Request->get_string('date_end_report', 'post,get', date("d/m/Y",NV_CURRENTTIME));
	if (defined('NV_IS_USER')  || defined('NV_IS_ADMIN')){
		if (defined('NV_IS_USER')){
			$row['userid'] = $user_info['userid'];
			$row['username'] = $user_info['username'];
		...
		/* foreach($users_reports as $key=>$users_report_tmp){ */
			//print_r($users_reports);
			$date_begin_array = explode("/",$date_begin_timekeeping);
			$start_date = $date_begin_array[2].'-' . $date_begin_array[1] . '-' . $date_begin_array[0] . '';
			$date_end_array = explode("/",$date_end_timekeeping);
			...
			foreach ($dates as $date) {
				...
				$date_current_end = mktime(23,59,59,$date_current_array[1],$date_current_array[0],$date_current_array[2]);
				//print_r($users_timekeeping);
				
				$row = $db->query('SELECT * FROM ' . NV_PREFIXLANG . '_' . $module_data . '_timekeeper WHERE userid=' . $users_reports . ' AND date_login >= ' . $date_current_start . ' AND date_login <= ' . $date_current_end);
				$data_array = array();
				 while($datas = $row->fetch()){
					$data_array[$datas['id']]=$datas;
				} 
				...
				foreach ($data_tmp as $key => $value){
					$value['fullname'] = $array_userid_users[$value['userid']]['last_name']  . ' ' . $array_userid_users[$value['userid']]['first_name'];
					$value['location'] = $data_work_place[$value['locationid']]['t'];
					$value['type_checkout'] = $data_array[$value['parentid']]['type_login'];
					$value['locationid_checkout'] = $data_array[$value['parentid']]['locationid'];
					$value['location_checkout'] = $data_work_place[$data_array[$value['parentid']]['locationid']]['t'];
					...
					$value['distance_checkout'] = $data_array[$value['parentid']]['distance'];
					$value['browse_checkout'] = $data_array[$value['parentid']]['browse'];
					//$value['duration'] = $value['time_checkout'] - $value['time_login'];
					$start_datetime = new DateTime($value['time_login']);
					$end_datetime = new DateTime($value['time_checkout']);

					// Tính khoảng cách giữa hai thời gian
					$time_difference = $start_datetime->diff($end_datetime);
					$value['duration'] = $time_difference->h . ':' . $time_difference->i;
					if(empty($value['time_checkout'])){$value['duration'] = 0;}
					$data[] = $value;
				}
				
				$users_report[$date] = array(
					'date' => $date,
					'data' => $data
				);
				
			}
			$json['data'][$users_reports]=$users_report;
