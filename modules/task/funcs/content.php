...
if ($row['id'] > 0) {
    ...
    if((nv_user_in_groups('12,13,17') and !in_array($user_info['userid'],$array_performer) ) and !nv_user_in_groups('1,2,3,10')){// Kiểm quyền
...
if ($nv_Request->isset_request('submit', 'post')) {
    ...
    try {
        ...
        if (empty($row['id'])) {
            $_sql = 'INSERT INTO ' . NV_PREFIXLANG . '_' . $module_data . ' (catid, title, performer, groups_made, begintime, exptime, description, useradd, projectid, addtime, edittime, status, priority, files, progress) VALUES (:catid, :title, :performer, :groups_made, :begintime, :exptime, :description, :useradd, :projectid, ' . NV_CURRENTTIME . ', ' . NV_CURRENTTIME . ', :status, :priority, :files, :progress)';
            $data_insert = array();
            ...
            $stmt = $db->prepare('UPDATE ' . NV_PREFIXLANG . '_' . $module_data . ' SET catid = :catid, title = :title, performer = :performer, groups_made = :groups_made, begintime = :begintime, exptime = :exptime, description = :description, edittime = ' . NV_CURRENTTIME . ', status = :status, priority = :priority, files = :files, useradd = :useradd, progress = :progress WHERE id=' . $row['id']);
...
$sql_project = '';
if($row['projectid'] > 0){
    $sql_project = ' WHERE projectid = ' . $row['projectid'];
}
// Danh sách hạng mục
$array_category = $db->query('SELECT id, title FROM ' . NV_PREFIXLANG . '_' . $module_data . '_cat' . $sql_project)->fetchAll();
if(!empty($array_category)){
    foreach ($array_category as $category) {
        $category['sl'] = ($category['id'] == $row['catid']) ? 'selected="selected"' : '';
