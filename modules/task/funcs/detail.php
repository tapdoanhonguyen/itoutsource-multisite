...
if ($nv_Request->isset_request('change_status', 'post')) {
	...
	$sql_update_progress = '';
	if($new_status == 2){
		$sql_update_progress = ' progress = 100,';
	}

    $sql = 'UPDATE ' . NV_PREFIXLANG . '_' . $module_data . ' SET status=' . $new_status . ',' . $sql_update_progress . ' edittime=' . NV_CURRENTTIME . ' ' . ($new_status == 2 ? ', realtime=' . NV_CURRENTTIME : '') . ' WHERE id=' . $id;
    $db->query($sql);
...
$array_performer = explode(',', $rows['performer']);
if(nv_user_in_groups('11') and !nv_user_in_groups('1,2,3,10,18') and !in_array($user_info['userid'], $array_performer) ){// Kiểm quyền
...
if ($rows['catid']) {
    $rows['hang_muc'] = $db->query('SELECT id, title FROM ' . NV_PREFIXLANG . '_' . $module_name . '_cat WHERE id=' . $rows['catid'])->fetch();
    $rows['hang_muc']['link'] = NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=task&amp;' . NV_OP_VARIABLE . '=detail-cat&id=' . $rows['hang_muc']['id'];
}
