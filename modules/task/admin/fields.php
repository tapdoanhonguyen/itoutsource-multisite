...
if ($nv_Request->isset_request('changestatus', 'post')) {
    ...
    $field = $nv_Request->get_title('field', 'post', 0);

    $query = 'SELECT ' . $field . ' FROM ' . NV_PREFIXLANG . '_' . $module_data . '_field WHERE fid=' . $fid;
    ...
    if ($numrows > 0) {
        ...
        $query = 'UPDATE ' . NV_PREFIXLANG . '_' . $module_data . '_field SET ' . $field . '=' . $db->quote($active) . ' WHERE fid=' . $fid;
        $db->query($query);
    }
...
if ($nv_Request->isset_request('choicesql', 'post')) {
    ...
    if ($choice == 'module') {
        ...
        if ($num_table > 0) {
            ...
            foreach ($_items as $item) {
                $_temp_choice['sl_key'] = (isset($choice_seltected[0]) and $choice_seltected[0] == $item['field']) ? ' selected="selected"' : '';
                $_temp_choice['sl_val'] = (isset($choice_seltected[1]) and $choice_seltected[1] == $item['field']) ? ' selected="selected"' : '';
...
if ($nv_Request->isset_request('submit', 'post')) {
    ...
    $dataform['sql_choices'] = $dataform['field_choices'] = '';
    ...
    if (empty($error)) {
        if (empty($dataform['fid'])) {
            $_columns_array = $db->columns_array(NV_PREFIXLANG . '_' . $module_data);
            ...
            if ($dataform['max_length'] <= 4294967296 and !empty($dataform['field']) and !empty($dataform['title']) and !isset($_columns_array[$dataform['field']])) {
                ...
                $sql = "INSERT INTO " . NV_PREFIXLANG . '_' . $module_data . "_field
					...
					required, show_profile, class, language, default_value) VALUES
                ...
                if ($dataform['fid']) {
                    ...
                    $save = $db->exec("ALTER TABLE " . NV_PREFIXLANG . '_' . $module_data . "_info ADD " . $dataform['field'] . " " . $type_date);
                }
            }
        } elseif ($dataform['max_length'] <= 4294967296) {
            ...
            if ($save and $dataform['max_length'] != $dataform_old['max_length']) {
                ...
                $save = $db->exec("ALTER TABLE " . NV_PREFIXLANG . '_' . $module_data . "_info CHANGE " . $dataform_old['field'] . " " . $dataform_old['field'] . " " . $type_date);
            }
        }
        if ($save) {
...
if ($nv_Request->isset_request('del', 'post')) {
    ...
    list ($fid, $field, $weight) = $db->query('SELECT fid, field, weight FROM ' . NV_PREFIXLANG . '_' . $module_data . '_field WHERE fid=' . $fid)->fetch(3);

    if ($fid and !empty($field)) {
        $query1 = 'DELETE FROM ' . NV_PREFIXLANG . '_' . $module_data . '_field WHERE fid=' . $fid;
        $query2 = 'ALTER TABLE ' . NV_PREFIXLANG . '_' . $module_data . '_info DROP ' . $field;
...
if ($nv_Request->isset_request('qlist', 'get')) {
    ...
    $sql = 'SELECT * FROM ' . NV_PREFIXLANG . '_' . $module_data . '_field ORDER BY weight ASC';
