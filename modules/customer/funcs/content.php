...
/*
// cập nhật thông tin mã khách hàng chưa có

$list_cus = $db->query('SELECT id FROM vidoco_vi_customer WHERE code IS NULL ORDER BY id ASC')->fetchAll();

foreach($list_cus as $cus)
{
// cập nhật thông tin code
$code = 'VIDOCO-' . $cus['id'];

$db->query('UPDATE vidoco_vi_customer SET code ="'. $code .'" WHERE id='. $cus['id']);
...
$row['id'] = intval(end($array_page));
$number = strlen($row['id']) + 1;
$token = substr($array_op[1], 0, -$number);
 
if( $token !=  md5( $nv_Request->session_id . $global_config['sitekey'] . $row['id'] ) && $row['id'] > 0 ) 
{
...
if( ! nv_user_in_groups( $array_config['groups_permission']['allow_edit'] ) and ! $is_add )
{
...
elseif( ! nv_user_in_groups( $array_config['groups_permission']['allow_add'] ) and $is_add )
{
	nv_redirect_location( nv_url_rewrite( NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $module_name , true ));
}
...
if( $nv_Request->isset_request( 'get_user_json', 'post, get' ) )
{
	...
	$db->sqlreset()->select( 'userid, first_name, last_name, username, email' )->from( NV_USERS_GLOBALTABLE )->where( '(first_name LIKE "%' . $q . '%"
            OR last_name LIKE "%' . $q . '%"
            OR username LIKE "%' . $q . '%"
            OR email LIKE "%' . $q . '%"
        ) AND userid NOT IN (SELECT userid_link FROM ' . NV_PREFIXLANG . '_customer)' )->order( 'first_name ASC' )->limit( 20 );
...
if( $row['id'] > 0 )
{
	...
	{
		$where_sql .= ' AND t1.userid_link = ' . $user_info['userid'];
	}
	else
	{
		if( ! defined( 'NV_IS_ADMIN' ) )
		{
			...
			$where_sql .= ' AND t2.userid=' . $user_info['userid'];
		}
		
	}

	$row = $db->query( 'SELECT t1.*, t2.customerid, t2.permisson FROM ' . NV_PREFIXLANG . '_' . $module_data . ' t1 INNER JOIN ' . NV_PREFIXLANG . '_' . $module_data . '_share_acc t2 ON t1.id=t2.customerid WHERE id=' . intval( $row['id'] ) . $where_sql . ' AND permisson=1' )->fetch();
...
$row['adduser_email'] = 1;
$row['share_default'] = array( $row['userid'], $row['care_staff'] );
...
if( $nv_Request->isset_request( 'submit', 'post' ) )
{
	...
	$row['share_customer'] = $nv_Request->get_int( 'share_customer', 'post', 0 );
	...
	if( ! empty( $row['unit'] ) )
	{
		if( ! is_numeric( $row['unit'] ) )
		{
			...
			$data_insert = array( 'title' => $row['unit'] );
	...
	elseif( empty( $row['id'] ) && ! empty( $row['main_email'] ) && $db->query( 'SELECT COUNT(*) FROM ' . NV_PREFIXLANG . '_' . $module_data . ' WHERE main_email=' . $db->quote( $row['main_email'] ) )->fetchColumn() > 0 )
	{
		$error[] = sprintf( $lang_module['error_exits_email'], $row['main_email'] );
	}
	elseif( empty( $row['id'] ) && ! empty( $row['main_phone'] ) && $db->query( 'SELECT COUNT(*) FROM ' . NV_PREFIXLANG . '_' . $module_data . ' WHERE main_phone=' . $db->quote( $row['main_phone'] ) )->fetchColumn() > 0 )
	{
	...
	if( empty( $error ) )
	{
		...
		try
		{
			...
			if( empty( $row['id'] ) )
			{
				$_sql = 'INSERT INTO ' . NV_PREFIXLANG . '_' . $module_data . ' (note, code, first_name, last_name, main_phone, other_phone, main_email, other_email, birthday, facebook, skype, zalo, website, gender, address, unit, care_staff, image, addtime, userid, is_contacts, type_id, tag_id,share_acc,share_groups, share_customer, userid_link) VALUES (:note, :code, :first_name, :last_name, :main_phone, :other_phone, :main_email, :other_email, :birthday, :facebook, :skype, :zalo, :website, :gender, :address, :unit, :care_staff, :image, ' . NV_CURRENTTIME . ', ' . $user_info['userid'] . ', :is_contacts, :type_id, :tag_id, :share_acc , :share_groups, :share_customer, :userid_link)';
				$data_insert = array();
				...
				$data_insert['share_customer'] = $row['share_customer'];
				$data_insert['userid_link'] = $row['userid_link'];

				$new_id = $db->insert_id( $_sql, 'id', $data_insert );

				// cập nhật điểm tích lũy cho khách hàng giới thiệu nà
				insert_tichluy( $new_id, $row['share_customer'] );

			}
			else
			{
				$stmt = $db->prepare( 'UPDATE ' . NV_PREFIXLANG . '_' . $module_data . ' SET code =:code, first_name = :first_name, last_name = :last_name, main_phone = :main_phone, other_phone = :other_phone, main_email = :main_email, other_email = :other_email, birthday = :birthday, facebook = :facebook, skype = :skype, zalo = :zalo, website = :website, gender = :gender, address = :address, unit = :unit, care_staff = :care_staff, image = :image, edittime=' . NV_CURRENTTIME . ', note = :note, type_id = :type_id, tag_id = :tag_id, share_acc = :share_acc, share_groups = :share_groups, share_customer =:share_customer, userid_link = :userid_link WHERE id=' . $row['id'] );
				...
				$stmt->bindParam( ':share_customer', $row['share_customer'], PDO::PARAM_INT );
				$stmt->bindParam( ':userid_link', $row['userid_link'], PDO::PARAM_INT );

				// cập nhật điểm tích lũy cho khách hàng giới thiệu nà
				update_tichluy( $row['id'], $row['share_customer'] );
			...
			if( $new_id > 0 )
			{
				...
				$array_userid = array( $user_info['userid'], $row['care_staff'] );
				...
				if( $row['care_staff'] != $row['care_staff_old'] )
				{
					...
					$array_userid = array( $row['care_staff'] );
...
$list_customer = $db->query( 'SELECT * FROM ' . NV_PREFIXLANG . '_' . $module_data . ' ORDER BY addtime DESC' )->fetchAll();

foreach( $list_customer as $customer )
{
	$xtpl->assign( 'customer', array(
		'key' => $customer['id'],
		'value' => $customer['last_name'] . ' ' . $customer['first_name'] . ' - ' . $customer['main_phone'],
		'selected' => ( $customer['id'] == $row['share_customer'] ) ? ' selected="selected"' : '' ) );

	$xtpl->parse( 'main.share_customer' );
