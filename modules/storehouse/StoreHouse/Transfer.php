...
class Transfer extends MY_Controller {
	...
	}	function add()    {    	if ($this->form_validation->run()) {    		$transfer_no = $this->input->get_title('reference_no','post','') ? $this->input->get_title('reference_no') : $this->site->getReference('to');			if (preg_match('/^([0-9]{1,2})\/([0-9]{1,2})\/([0-9]{4})$/', $this->input->get_string('date', 'post', 0), $m))     {		        $_hour = $this->input->get_int('date_hour', 'post');		        $_min = $this->input->get_int('date_min', 'post');		        $date = mktime($_hour, $_min, 0, $m[2], $m[1], $m[3]);		    }		    else		    {		        $date = 0;		    }			$to_warehouse = $this->input->get_int('warehouse_id_new');            $from_warehouse = $this->input->get_int('warehouse_id');			$from_warehouse_details = $this->site->getWarehouseByID($from_warehouse);            $from_warehouse_code = $from_warehouse_details->code;            $from_warehouse_name = $from_warehouse_details->name;			$to_warehouse_details = $this->site->getWarehouseByID($to_warehouse);            $to_warehouse_code = $to_warehouse_details->code;            $to_warehouse_name = $to_warehouse_details->name;			$note = $this->sma->clear_tags($this->input->get_title('note','post',''));            $shipping = $this->input->get_title('shipping','post','') ? $this->input->get_title('shipping','post','') : 0;            $status = $this->input->get_int('status','post',0);			$total = 0;            $product_tax = 0;            $gst_data = array();            $total_cgst = $total_sgst = $total_igst = 0;            $i = sizeof($this->input->get_array('product_code','post', 0));			$product_id = $this->input->get_array('product_id', 'post', 0);		    $product_quantity = $this->input->get_array('product_quantity', 'post', 0);		    $product_option = $this->input->get_array('product_option', 'post', 0);		    $product_expried = $this->input->get_array('product_expried', 'post', 0);		    $product_discount_array = $this->input->get_array('product_discount', 'post', 0);		    $product_tax_rate = $this->input->get_array('product_tax_rate', 'post', 0);		    $product_tax_array = $this->input->get_array('product_tax', 'post', 0);		    $product_cost_tax = $this->input->get_array('product_cost_tax', 'post', 0);		    $product_total = $this->input->get_array('product_total', 'post', 0);		    $product_code = $this->input->get_array('product_code', 'post', 0);		    $product_name = $this->input->get_array('product_name', 'post', 0);		    $product_unit = $this->input->get_array('product_unit', 'post', 0);		    $product_base_quantity = $this->input->get_array('product_base_quantity', 'post', 0);		    $product_real_unit_cost = $this->input->get_array('product_real_unit_cost', 'post', 0);		    $product_net_cost = $this->input->get_array('product_net_cost', 'post', 0);		    $product_unit_cost = $this->input->get_array('product_unit_cost', 'post', 0);		    $part_no = $this->input->get_array('part_no', 'post', 0);			$product_tax = 0;			$product_discount = 0;			for ($r = 0; $r < $i; $r++) {				$item_code = $product_code[$r];				$item_net_cost = $this -> sma -> formatDecimal($product_net_cost[$r]);				$unit_cost = $this -> sma -> formatDecimal($product_unit_cost[$r]);				$real_unit_cost = $this -> sma -> formatDecimal($product_real_unit_cost[$r]);				$item_unit_quantity = $product_base_quantity[$r];				$item_option = isset($product_option[$r]) && $product_option[$r] != 'false' ? $product_option[$r] : null;				$item_tax_rate = isset($product_tax_array[$r]) ? $product_tax_array[$r] : null;				$item_discount = isset($product_discount_array[$r]) ? $product_discount_array[$r] : null;				$item_expiry = (isset($product_expried[$r]) && !empty($product_expried[$r])) ? $product_expried[$r] : '';				$supplier_part_no = (isset($part_no[$r]) && !empty($part_no[$r])) ? $part_no[$r] : null;				$item_unit = $product_unit[$r];				$item_quantity = $product_base_quantity[$r];				if (isset($item_code) && isset($real_unit_cost)  && isset($item_quantity)) {					$product_details = $this -> site -> getProductByCode($item_code);					$pr_discount = $this -> site -> calculateDiscount($item_discount, $unit_cost);					$unit_cost = $this -> sma -> formatDecimal($unit_cost - $pr_discount);					$item_net_cost = $unit_cost;					$pr_item_discount = $this -> sma -> formatDecimal($pr_discount * $item_unit_quantity);					$product_discount += $pr_item_discount;					$pr_item_tax = $item_tax = 0;					$tax = "";					if (isset($item_tax_rate) && $item_tax_rate != 0) {						$tax_details = $this -> site -> getTaxRateByID($item_tax_rate);						$ctax = $this -> site -> calculateTax($product_details, $tax_details, $unit_cost);						$item_tax = $ctax['amount'];						$tax = $ctax['tax'];						if ($product_details -> tax_method != 1) {							$item_net_cost = $unit_cost - $item_tax;						}						$pr_item_tax = $this -> sma -> formatDecimal($item_tax * $item_unit_quantity, 4);						if ($this -> Settings -> indian_gst && $gst_data = $this -> gst -> calculteIndianGST($pr_item_tax, ($this -> Settings -> state == $supplier_details -> state), $tax_details)) {							$total_cgst += $gst_data['cgst'];							$total_sgst += $gst_data['sgst'];							$total_igst += $gst_data['igst'];						}					}					$product_tax += $pr_item_tax;					$subtotal = (($item_net_cost * $item_unit_quantity) + $pr_item_tax);					$unit = $this -> site -> getUnitByID($item_unit);					$product = array(                        'product_id' => $product_details->id,                        'product_code' => $item_code,                        'product_name' => $product_details->name,                        'option_id' => $item_option,                        'net_unit_cost' => $item_net_cost,                        'unit_cost' => $this->sma->formatDecimal($item_net_cost + $item_tax, 4),                        'quantity' => $item_quantity,                        'product_unit_id' => $item_unit,                        'product_unit_code' => $unit->code,                        'unit_quantity' => $item_unit_quantity,                        'quantity_balance' => $item_quantity,                        'warehouse_id' => $to_warehouse,                        'item_tax' => $pr_item_tax,                        'tax_rate_id' => $item_tax_rate,                        'tax' => $tax,                        'subtotal' => $this->sma->formatDecimal($subtotal),                        'expiry' => $item_expiry,                        'real_unit_cost' => $real_unit_cost,                        'date' => $date                    );					$products[] = ($product + $gst_data);					$total += $this -> sma -> formatDecimal(($item_net_cost * $item_unit_quantity), 4);				}			}			$grand_total = $this->sma->formatDecimal(($total + $shipping + $product_tax), 4);			$data = array('transfer_no' => $transfer_no,                'date' => $date,                'from_warehouse_id' => $from_warehouse,                'from_warehouse_code' => $from_warehouse_code,                'from_warehouse_name' => $from_warehouse_name,                'to_warehouse_id' => $to_warehouse,                'to_warehouse_code' => $to_warehouse_code,                'to_warehouse_name' => $to_warehouse_name,                'note' => $note,                'total_tax' => $product_tax,                'total' => $total,                'grand_total' => $grand_total,                'created_by' => $this->session->userdata('user_id'),                'status' => $status,                'shipping' => $shipping            );            if ($this->Settings->indian_gst) {                $data['cgst'] = $total_cgst;                $data['sgst'] = $total_sgst;                $data['igst'] = $total_igst;            }		}		if ($this->form_validation->run() == true && $this->transfer_model->addTransfer($data, $products)) {			return true;		}	}	public function AddItems($row = array()) {
