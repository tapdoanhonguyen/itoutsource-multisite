                ...
                if ($status == 4) {
                    ...
                    try {
			            if (empty($item['id'])) {
			                ...
			                $stmt = $db->prepare('UPDATE ' . $this -> db_prefix . '_' . $this -> mod_data . '_transfer_items SET transfer_id = :transfer_id, product_id = :product_id, product_code = :product_code, product_name = :product_name, option_id = :option_id, expiry = :expiry, quantity = :quantity, tax_rate_id = :tax_rate_id, tax = :tax, item_tax = :item_tax, net_unit_cost = :net_unit_cost, subtotal = :subtotal, quantity_balance = :quantity_balance, unit_cost = :unit_cost, real_unit_cost = :real_unit_cost, date = :date, warehouse_id = :warehouse_id, product_unit_id = :product_unit_id, product_unit_code = :product_unit_code, unit_quantity = :unit_quantity, gst = :gst, cgst = :cgst, sgst = :sgst, igst = :igst WHERE id=' . $row['id']);
			            }
			            $stmt->bindParam(':transfer_id', $item['transfer_id'], PDO::PARAM_INT);
			            $stmt->bindParam(':product_id', $item['product_id'], PDO::PARAM_INT);
			            $stmt->bindParam(':product_code', $item['product_code'], PDO::PARAM_STR);
