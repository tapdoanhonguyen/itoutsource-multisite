...
class Site extends Model
{
    ...
    public function get_total_qty_alerts() {
        ...
        return $this->db->query('SELECT * FROM ' . $this->db_prefix . '_' . $this->mod_data . '_products ' . $where)->rowCount(); 
    }
    ...
    public function get_setting() {
		$q = $this->db->query('SELECT * FROM ' . $this->db_prefix . '_' . $this->mod_data . '_settings ');
...
/*         $q = $this->db->get_where('companies', array('group_name' => $group_name));
    ...
    public function getCompanyByID($id) {
        $q = $this->db->query('SELECT * FROM ' . $this->db_prefix . '_' . $this->mod_data . '_companies WHERE id = ' . $id);
...
/*         if (!$id) {
    ...
    public function getProductByID($id) {
         $q = $this->db->query('SELECT * FROM ' . $this->db_prefix . '_' . $this->mod_data . '_products WHERE id = ' . $id);
        if ($q->rowCount() > 0) {
            return $q->fetch(PDO::FETCH_OBJ);
        } 
        return FALSE;
...
/*         $q = $this->db->get('tax_rates');
    ...
    public function getTaxRateByID($id) {
         $q = $this->db->query('SELECT * FROM ' . $this->db_prefix . '_' . $this->mod_data . '_tax_rates WHERE id=' . $id);
        if ($q->rowCount() > 0) {
            return $q->fetch(PDO::FETCH_OBJ);
        }
        return FALSE;
    ...
    public function getWarehouseByID($id) {
        $q = $this->db->query('SELECT * FROM ' . $this->db_prefix . '_' . $this->mod_data . '_warehouses WHERE id=' . $id);
        if ($q->rowCount() > 0) {
            return $q->fetch(5);
        } 
        return FALSE;
...
/*         $date = date('Y-m-d');
    ...
    public function updateReference($field) {
        ...
        if ($q->rowCount() > 0) {
            ...
            $this->db->query ('UPDATE ' . $this->db_prefix . '_' . $this->mod_data . '_order_ref SET ' . $field . ' = ' .   $num_field  . '  WHERE ref_id = 1');
            return TRUE;
        } 
        return FALSE;
    }
    ...
    public function checkPermissions() {
        $q = $this->db->query('SELECT * FROM ' . $this->db_prefix . '_' . $this->mod_data . '_permissions WHERE group_id =' . $this->session->userdata($this->mod_data .'_group_id'));
        if ($q->rowCount() > 0) {
            return $q->fetch(PDO::FETCH_OBJ);
        } 
        return FALSE;
...
/*         if (!$user_id) {
   ...
   /*      if ($warehouse_id) {
    ...
    public function getPurchasedItem($clause) {
		 ...
		 ->from($this->db_prefix . '_' . $this->mod_data . '_purchase_items');
    ...
    public function syncProductQty($product_id, $warehouse_id) {
        ...
        if ($this->db->query('UPDATE ' . $this->db_prefix . '_' . $this->mod_data . '_products SET quantity = ' . $balance_qty . ' WHERE id =' . $product_id)) {
            if ($this->getWarehouseProducts($product_id, $warehouse_id)) {
                $this->db->query('UPDATE ' . $this->db_prefix . '_' . $this->mod_data . '_warehouses_products SET quantity = ' . $wh_balance_qty. ' WHERE product_id =' . $product_id . ' AND warehouse_id = ' . $warehouse_id);
            } else {
                if( ! $wh_balance_qty) { $wh_balance_qty = 0; }
                $product = $this->site->getProductByID($product_id);
                $this->db->query('INSERT INTO ' . $this->db_prefix . '_' . $this->mod_data . '_warehouses_products SET quantity = ' . $wh_balance_qty . ', product_id = ' . $product_id . ', warehouse_id = ' . $warehouse_id . ', avg_cost = ' . $product->cost);
            }
            return TRUE;
        } 
        return FALSE;
 ...
 */
    ...
    private function getBalanceQuantity($product_id, $warehouse_id = NULL) {
        ...
        $where = 'product_id = '. $product_id .' AND quantity_balance !=0 ';
        if ($warehouse_id) {
            $where .= 'AND warehouse_id = ' . $warehouse_id;
        }
    ...
    public function getPurchasedItems($product_id, $warehouse_id = NULL, $option_id = NULL) {
		...
		$this->db->from($this->db_prefix . '_' . $this->mod_data . '_purchase_items');
    ...
    public function getAllSaleItems($sale_id) {
         $q = $this->db->query('SELECT * FROM ' . $this->db_prefix . '_' . $this->mod_data . '_sale_items WHERE sale_id = ' . $sale_id);
        if ($q->rowCount() > 0) {
            return $q->fetchAll(5);
		}
        return FALSE;
    ...
    public function getAllPurchaseItems($purchase_id) {
         $q = $this->db->query('SELECT * FROM ' . $this->db_prefix . '_' . $this->mod_data . '_purchase_items WHERE purchase_id = ' . $purchase_id);
        if ($q->rowCount() > 0) {
            return $q->fetchAll(5);
        } 
        return FALSE;
                ...
                foreach ($items as $item) {
                    if (isset($item['pi_overselling'])) {
                        ...
                        if ($pi = $this->getPurchasedItem($clause)) {
                            ...
                            $this->db->query('UPDATE ' . $this->db_prefix . '_' . $this->mod_data . '_purchase_items SET quantity_balance =' . $quantity_balance .' WHERE id=' . $pi->id);
                        } else {
                            $clause['quantity'] = 0;
                            $clause['item_tax'] = 0;
                            ...
                            $clause['status'] = 4;
                            $clause['option_id'] = !empty($clause['option_id']) && is_numeric($clause['option_id']) ? $clause['option_id'] : NULL;
                            $this->db->query('INSERT INTTO ' . $this->db_prefix . '_' . $this->mod_data . '_purchase_items (quantity, item_tax, quantity_balance, status, option_id) VALUES ( '. $clause['quantity'] . ',' . $clause['item_tax'] . ',' . $clause['quantity_balance'] . ',' . $clause['status'] . ',' . $clause['option_id'] . ')');
                        }
                    } else {
                        if ($item['inventory']) {
                            $this->db->query('UPDATE ' . $this->db_prefix . '_' . $this->mod_data . '_purchase_items SET quantity_balance =' . $item['quantity_balance'] .' WHERE id=' . $item['purchase_item_id']);
                        }
                    }
                }
            }
    ...
    public function getUnitByID($id) {
		$q = $this->db->query('SELECT * FROM ' . $this->db_prefix . '_' . $this->mod_data . '_units WHERE id = ' . $id);
	...
	public function getSalesItems($product_id, $warehouse_id= NULL, $option_id = NULL) {
			...
			$this->db->from($this->db_prefix . '_' . $this->mod_data . '_sale_items t1')
			->join('LEFT JOIN ' . $this->db_prefix . '_' . $this->mod_data . '_sales t2 ON t1.sale_id = t2.id');
