...
if( ! nv_function_exists( 'creat_thumbslider' ) )
{
	function creat_thumbslider( $id, $homeimgfile, $module_name, $width = 200, $height = 150 )
	{
...
if (! nv_function_exists('nv_block_slider')) {
    ...
    {
        global $global_config, $site_mods, $module_config, $lang_global, $nv_Cache, $db;
        ...
        $sql = 'SELECT id, title, image, link_href, link_target FROM ' . NV_PREFIXLANG . '_' . $site_mods[$module]['module_data'] . '_rows WHERE status = 1 AND bid = ' . $block_config['blockid']. ' ORDER BY id ASC';
            ...
            foreach ($list as $row) {
                if (! empty($row['image'])) {
                   $row['image'] = NV_BASE_SITEURL . NV_UPLOADS_DIR . '/' . $site_mods[$module]['module_upload'] . '/' . $row['image'];
					//$row['image'] = creat_thumbslider( $row['id'], $row['image'], $module, 1500, 661 );
                }
