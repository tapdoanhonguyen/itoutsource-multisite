 ...
 * @return string
 */
function nv_smartlineco_menu_blocks($block_config)
{
    ...
    $list = $nv_Cache->db($sql, '', 'smartlineco_' . $block_config['module']);
    ...
    foreach ($list as $row) {
        if (nv_user_in_groups($row['groups_view'])) {
            ...
            $list_cats[$row['id']] = [
                ...
                'icon' => $row['icon'],
                'html_class' => $row['css'],
                'current' => nv_menu_smartlineco_check_current($row['link'], $row['active_type'])
            ];
        }
    }
        ...
        foreach ($list_cats as $cat) {
            if (empty($cat['parentid'])) {
                if (!empty($cat['subcats'])) {
                    $submenu_active = [];
                    $html_content = nv_smartlineco_smenu_blocks($block_config['block_name'], $list_cats, $cat['subcats'], $submenu_active, $block_theme);
                ...
                $cat['class'] = nv_smartlineco_menu_blocks_active($cat);
 ...
 * @return string
 */
function nv_smartlineco_menu_blocks_active($cat)
{
 ...
 * @return bool
 */
function nv_menu_smartlineco_check_current($url, $type = 0)
{
 ...
 * @return string
 */
function nv_smartlineco_smenu_blocks($style, $list_cats, $list_sub, &$submenu_active, $block_theme)
{
    ...
    foreach ($list_cats as $cat) {
        ...
        if (in_array((int) $catid, $list, true)) {
            $list_cats[$catid]['class'] = nv_smartlineco_menu_blocks_active($list_cats[$catid]);
            ...
            if (!empty($list_cats[$catid]['subcats'])) {
                $tree = nv_smartlineco_smenu_blocks($style, $list_cats, $list_cats[$catid]['subcats'], $submenu_active, $block_theme);
