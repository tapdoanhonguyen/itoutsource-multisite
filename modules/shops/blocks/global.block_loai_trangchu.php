    ...
    function nv_relates_product3($block_config)
    {
		...
		if(!empty($block_config['catid']))
		{
			foreach($block_config['catid'] as $bid)
			{
				...
				->where('t2.catid= ' . $bid . ' AND t1.status =1')
						...
						if ($pro_config['active_price'] == '1') {
							if ($row['showprice'] == '1') {
								...
								if ($row['discount_id'] and $price['discount_percent'] > 0) {
									...
									$xtpl->parse('main.loopcatid1.loop.price.discounts');
								} else {
									$xtpl->parse('main.loopcatid.loop.price.no_discounts');
									$xtpl->parse('main.loopcatid1.loop.price.no_discounts');
								}
								$xtpl->parse('main.loopcatid.loop.price');
								$xtpl->parse('main.loopcatid1.loop.price');
							} else {
								$xtpl->parse('main.loopcatid.loop.contact');
								$xtpl->parse('main.loopcatid1.loop.contact');
							}
						}
						...
						if ($pro_config['active_wishlist']) {
							if (! empty($array_wishlist_id)) {
								if (in_array($row['id'], $array_wishlist_id)) {
									...
									$xtpl->parse('main.loopcatid1.loop.wishlist.disabled');
								}
							}
