...
if (!nv_function_exists('nv_block_partner')) {
    /**
     ...
     * @return
     */
    function nv_block_config_partner($module, $data_block, $lang_block)
    {
		...
		if( defined( 'NV_EDITOR' ) )
			...
			$partner = htmlspecialchars( nv_editor_br2nl( $data_block['partner'] ) );
		    if( defined( 'NV_EDITOR' ) and nv_function_exists( 'nv_aleditor' ) )
		    {
			$partner = nv_aleditor( 'config_partner', '100%', '150px', $partner, 'Basic' );
		    }
		    else
		    {
			$partner = '<textarea class=\"form-control w500\" name="config_partner" id="config_partner" cols="20" rows="8">' . $partner . '</textarea>';
		    }
		...
		$html .= '<div class="form-group">';
			...
			$html .= '<div class="form-group">';
	        $html .= '<label class="control-label col-sm-6">' . $lang_block['partner'] . ':</label>';
	        $html .= '<div class="col-sm-18">' . $partner . '</div>';
	        $html .= '</div>';
     ...
     * @return
     */
    function nv_block_config_partner_submit($module, $lang_block)
    {
        global $nv_Request;
		$partner = $nv_Request->get_editor( 'config_partner', '', NV_ALLOWED_HTML_TAGS );
		$partner = strtr( $partner, array(
			"\r\n" => '',
			"\r" => '',
			"\n" => '' ) );
        $return = array();
     ...
     * @return
     */
    function nv_block_partner($block_config)
    {
        ...
        if (!empty($list)) {
            if (file_exists(NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/' . $site_mods[$module]['module_file'] . '/block.partner.tpl')) {
                $block_theme = $global_config['module_theme'];
            } elseif (file_exists(NV_ROOTDIR . '/themes/' . $global_config['site_theme'] . '/modules/' . $site_mods[$module]['module_file'] . '/block.partner.tpl')) {
			...
			$partner = strip_tags($block_config['partner']);
            //$partner = nv_clean60($partner, 300);
			$xtpl->assign( 'partner', $partner );
...
if (defined('NV_SYSTEM')) {
    $content = nv_block_partner($block_config);
}
